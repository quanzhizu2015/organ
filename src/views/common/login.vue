    ...
    <div class="login-container">
      <el-col class="title-bg" :span="8">
        <el-col class="title-content">
          <div>
            ...
            <div><span class="title">机构管理系统</span></div>
          </div>
          <span class="content"
      ...
      <el-col class="contain-right" :span="16">
        <el-form
          ...
          label-width="100px"
        >
          <el-form-item prop="account" label="账号" style="margin-bottom: 35px">
            <el-input
              type="text"
...
export default {
  ...
  methods: {
    ...
    getImgCode() {
      ...
      // }toparts-app-sys/validateImageServlet?radmon=${new Date().getTime()}`;
      // /train/admin/imgValidateCode

      this.$axios.get("/train/admin/imgValidateCode", {}).then((res) => {
        if (parseInt(res.code) === 0) {
          this.imgCode = res.data.img;
          this.imgCodeKey = res.data.imgCodeKey;
        } else {
      ...
      if (realRoutes && realRoutes.length > 0) {
        for (let i = 0; i < realRoutes.length; i++) {
          ...
          if (parentRouter.children && parentRouter.children.length > 0) {
            ...
            console.log(JSON.stringify(parentRouter.children[0]));
            break;
          }
        }
      }
          ...
          this.$store
            ...
            .then((res) => {
              ...
              if (res.data.menu && res.data.menu.length > 0) {
                path = this.findPath(res.data.menu);
                this.$axios
                  .post("train/schoolWeb/getSchoolInfo", {})
                  .then((respond) => {
...
.loginBj {
  ...
  .login-container {
    ...
    .title-bg {
      //background: url(../../assets/title-bg.png) no-repeat center center;
