<template>
  <div>
    <div class="ta-l"><span>图文导航</span></div>
    <div
      class="flex row ai-c bo-b3 pd-1 mt-1"
      v-for="item in value.divDatas"
      :key="item.id + item.url"
    >
      <img class="w-3 h-3 of-co" :src="item.url" @click="imageReselect(item)" />
      <div class="ml-02">
        <el-input
          size="small"
          v-model="item.title"
    ...
    <div>
      <span class="tx-008 tx-wb txc-60"
        >上传建议：尺寸为1：1，png、jpg、bmp、gif格式，不大于2M。</span
      >
    </div>
    ...
    <SelectMenuImageDialog
      ...
      @addSuccess="addMenuImageSuccess"
    >
    </SelectMenuImageDialog>
  </div>
</template>
