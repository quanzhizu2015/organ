    ...
    <div class="flex row jc-sb mt-1">
      <div class="bo-b3 catory">
        <div class="flex row ai-c mb-1 jc-sb">
          <span class="tx-1 tx-wb txc-60">图片分组</span>
      ...
      <div class="imageTableContain ml-1">
        ...
        >
          ...
          <el-table-column label="图片名称" width="400">
            <template slot-scope="scope">
              <el-image
                class="w-6 h-4 of-co"
                :src="rzImageUlr(scope.row.url, 200, 100)"
...
import ChangeTitleDialog from "../../components/changeTitleDialog";
    ...
    getGroup() {
      this.$axios
        .get(this.API.newNetSchool.material.groupSelect, {
          params: { name: this.catoryName, type: 1 },
        })
        .then((res) => {
          let records = [];
          res.data.forEach((element) => {
            records.push({
              ...element,
              isOver: false,
              isSelected: element.id == 0 ? true : false,
            });
          });
